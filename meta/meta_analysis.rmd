...
output:
  ...
  word_document:
    ...
    highlight: tango
---
The essence of the wholistic analysis  process is answering the research question (making inferences) by the sequence of wholeness-extending  transformations (explained in more detail elsewhere (*TODO: link*)). Due to computational intensity, such analysis process would benefit enourmously from the availability of a support tool. The ecosystem for conducting analyses with the tool is described below first and then the implemented transformations
...
## Observations
Analyses are based on observations. These should be maintained in a very flexible manner, 
to accommodate the changes. Like presented in the table below:
```{r echo = FALSE}
