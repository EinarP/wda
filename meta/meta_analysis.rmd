...
output:
  ...
  word_document:
    ...
    highlight: tango
---
An authoritative source (The BABOK Guide [BABOK_Guide_v3_Member.pdf]) defines business analysis as practice of enabling change in an enterprise by defining needs and recommending solutions that deliver value to stakeholders. Sequential business analysis (SBA) method proposes to clarify these needs and solutions iteratively through sequences of analysis steps. This method (explained in more detail elsewhere (*TODO: link*)) is heavily influenced by late Christopher Alexander's architectural theories [NoO] and are expected to facilitate particularly adaptive analyses. As generating sequences can be very laborious, such analysis process is practically unthinkable without proper support tools. Principles of such a tool (the analysis ecosystem) is described below first and then the implemented fundamental sequence steps (transformations). Conduction of analyses is illustrated by the tool usage meta-analysis
...
tbl <- rbind(c('O', 'O',	'NA', 'NA', 'O', 'NA', 'NA', 'NA', 'NA'),
