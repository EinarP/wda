...
output:
  ...
  word_document:
    ...
    highlight: tango
---
An authoritative source (The BABOK Guide [BABOK_Guide_v3_Member.pdf]) presents business analysis as the practice of enabling change in an enterprise by defining needs and recommending solutions that deliver value to stakeholders. Sequential business analysis (SBA) method proposes to clarify these needs and solutions iteratively through sequences of analysis steps. This method (explained in more detail elsewhere (*TODO: link*)) is heavily influenced by late Christopher Alexander's architectural theories [NoO] and are expected to facilitate particularly adaptive analyses. As generation of sequences can be very laborious, such analysis process is practically unthinkable without proper support tools. Principles of such a tool (the analysis ecosystem) is described below first and then the implemented fundamental sequence steps (transformations). Conduction of analyses is illustrated by the tool usage meta-analysis
...
## Observations
Analyses are based on specifically organised data (as observations). The proposed organisation is expected to be flexible enough for supporting majority of business analysis needs. Observations have the following components:
...
tbl <- matrix(c('Object', "Things (nested hierarchy levels separated by '>') or links (entities separated by '|') between them", 'OBSERVATION>property, VERTEX>id|OBSERVATION'), nrow = 1) %>%
  ...
  rbind(c('Source', 'Where the observation came from (optional)', 'url, book reference'))
...
```{r}
obs('checkpoint:entity(=instance)>attribute(>property)=value{source}')
```
