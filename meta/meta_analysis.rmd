...
output:
  ...
  word_document:
    ...
    highlight: tango
---
This document describes how to use the tool for supporting a wholistic data analysis process
...
# About
Wholistic data analysis is inspired by the work of architect Christopher Alexander and is presented in more detail elsewhere (*TODO: link*). This process would benefit enourmously from the availability of a support tool. Such tool as a collection of R functions is described below

```{r}
# TODO: First rename to wda, then split to multiple files and package
source('../strfa.R')
```

# The Analysis Ecosystem
In a nutshell, wholistic analysis means answering the research question (making inferences) by the sequence of wholeness-extending  transformations. The ecosystem for conducting such process is described below

## Observations
Analyses are based on observations. These are data collected on object property values. Objects can be either entities or links between them. Hierarchy of entities can be specified by using '>' symbol to separate the levels. Links between entities contain linked entities separated with '|' symbol.

Can be presented as a table in universal long format:

```{r echo = FALSE}
obsf <- data.frame(list(x = '*Definition*',
  object = 'Entity or link between entities',
  property = 'Value or meta  characteristic',
  value = 'Property value',
  checkpoint = 'Notional or temporal event of measuring',
  source = 'Where the observation came from'))
obsf <- rbind(obsf, data.frame(list(x = '*Example*',
  object = 'levelx>group>entity>attribute',
  property = 'value, type',
  value = '10, numeric',
  checkpoint = 'DISBURSEMENT, 2018-03-03T09:57:10',
  source = 'gl, book reference')))
kable(obsf)
```
...
Observations can be assigned to a variable (like in a code snippet below) or added directly during any transformation
...
meta <- applySeed(sq = meta, seed = 29)
...
meta <- applySeed(sq=meta, seed=725)
meta
```
