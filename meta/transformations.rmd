...
```{r}
# Because of the bug cannot add attributes immideatly
tdx <- grow(td, 'SYMMETRICAL_1')
grow(tdx, 'SYMMETRICAL_1', attrs=T)
```

## <span style="font-variant:small-caps;">Gradients</span> transformation

```{r}
# Because of the bug cannot add attributes immediately
tdx <- applyAlternation(grow(td, 'G_FOCUS'))
tdx <- grow(tdx, 'G_FOCUS', depth=2, attrs=T)
